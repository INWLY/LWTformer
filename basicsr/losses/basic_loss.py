...
class CompositeLoss(nn.Module):
    ...
    def __init__(self, l1_weight=1.0, fft_weight=1.0, reduction='mean'):
        super().__init__()
        self.l1_loss = L1Loss(loss_weight=l1_weight, reduction=reduction)
        self.fft_loss = FFTLoss(loss_weight=fft_weight, reduction=reduction)

    def forward(self, pred, target, weight=None, **kwargs):
        l1 = self.l1_loss(pred, target, weight, **kwargs)
        fft = self.fft_loss(pred, target, weight, **kwargs)
        return l1 + fft  # 自动应用各自的loss_weight
